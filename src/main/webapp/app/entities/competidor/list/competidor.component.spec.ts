import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { of } from 'rxjs';

import { CompetidorService } from '../service/competidor.service';

import { CompetidorComponent } from './competidor.component';

describe('Competidor Management Component', () => {
  let comp: CompetidorComponent;
  let fixture: ComponentFixture<CompetidorComponent>;
  let service: CompetidorService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule],
      declarations: [CompetidorComponent],
    })
      .overrideTemplate(CompetidorComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(CompetidorComponent);
    comp = fixture.componentInstance;
    service = TestBed.inject(CompetidorService);

    const headers = new HttpHeaders();
    jest.spyOn(service, 'query').mockReturnValue(
      of(
        new HttpResponse({
          body: [{ id: 123 }],
          headers,
        })
      )
    );
  });

  it('Should call load all on init', () => {
    // WHEN
    comp.ngOnInit();

    // THEN
    expect(service.query).toHaveBeenCalled();
    expect(comp.competidors?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });
});
